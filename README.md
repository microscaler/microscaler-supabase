...
Unified Supabase stack for Microscaler services (PriceWhisperer, seasame-idam, etc.). Fully externalised; use as a **side clone** (or later as a submodule).
...
Use as a **side clone** next to PriceWhisperer or seasame-idam:
...
1. **Secrets:** `k8s/data/deployment-configuration/profiles/dev/application.secrets.env` is included with plaintext dev values. For production, replace with SOPS-encrypted or copy from PriceWhisperer. See `application.secrets.env.example` for required keys.
2. **Node name:** Edit `k8s/data/persistent-volumes.yaml` and set nodeAffinity hostname to your Kind node (e.g. `pricewhisperer-control-plane`, `sesame-idam-control-plane`, or `supabase-control-plane`).
...
Moved from PriceWhisperer: docker/postgres, k8s/data (postgres stack, deployment-configuration subset), scripts (build-postgres-docker.sh, setup-supabase-users.sh, sql/).
