...
- **.github/workflows/ci.yml** — Octopilot pipeline: `detect-contexts` → `lint` → `test` (Helm/Kind) → UUID for ttl.sh → matrix `integration-build-artifact` (Docker + chart) → `merge-build-results` verify.
